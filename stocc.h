...
#ifdef PY_BUILD
    #include "stocPy.h"           // Include this when building Python interface
#endif
...
public:
    CWalleniusNCHypergeometric(int n, int m, int N, double odds, double accuracy=1.E-8); // constructor
    void SetParameters(int n, int m, int N, double odds); // change parameters
    double probability(int x);                            // calculate probability function
    int MakeTable(double * table, int MaxLength, int * xfirst, int * xlast, double cutoff = 0.); // make table of probabilities
    double mean(void);                                    // approximate mean
    double variance(void);                                // approximate variance (poor approximation)
    int mode(void);                                       // calculate mode
    double moments(double * mean, double * var);          // calculate exact mean and variance
    int BernouilliH(int x, double h, double rh, StochasticLib1 *sto); // used by rejection method
...
protected:
    ...
    // parameters
    double omega;                                           // Odds
    int n, m, N, x;                                         // Parameters
    int xmin, xmax;                                         // Minimum and maximum x
